...
def click_save(frame_lable, str_code, str_mark, str_model, str_producting_year, str_price, variable, r_var):
    condition = "" 
    if (r_var.get()):
        condition = "new"
    else:
...
create = Button(frame_btn, text="Добавление записи", background="maroon", foreground="white", padx="20", pady="8", font=("Courier", 10), command=lambda: click_save(frame_lable,str_code, str_mark, str_model, str_producting_year, str_price, variable, r_var)).grid(row=0, column=0)
